      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            style={{ width: "min(420px, 100vw)", background: "#111C2B", borderLeft: "1px solid rgba(255,255,255,0.07)" }}
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
            ...
            {items.length > 0 && (
              <div className="px-6 py-5" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => { alert("Bedankt voor je bestelling! We bereiden je poke bowl zo snel mogelijk."); clearCart(); closeCart(); }}
                >
                  Bestelling Plaatsen →
                </motion.button>
                <button
